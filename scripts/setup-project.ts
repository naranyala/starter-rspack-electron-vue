...
async function installPackage(
  packageName: string,
  options: { force?: boolean; dev?: boolean; global?: boolean } = {}
): Promise<boolean> {
  ...
  try {
    ...
    logError(`Failed to install ${packageName}: ${(error as Error).message}`);
    return false;
  }
}
...
async function ensurePackageManager(): Promise<void> {
...
function validateProjectStructure(): void {
  const requiredFiles = ['package.json', 'main.cjs', 'src/renderer/components/App.vue'];
...
async function setupProject(): Promise<void> {
  try {
    ...
    logError(`Project setup failed: ${(error as Error).message}`);
    process.exit(1);
  }
}
...
export { setupProject, installPackage, ensurePackageManager };
